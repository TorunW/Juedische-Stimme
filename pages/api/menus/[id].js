...
import { updateTermTaxonomy, updateTermMeta,  deleteTerm, deleteTermRelationship, deleteTermTaxonomy, deleteTermMeta, insertTerm, insertTermRelationship, insertTermTaxonomy, insertTermMeta } from 'lib/queries';
    ...
    try {
        if (req.method === "PUT"){
            ...
            const updateTermMetaResult = await excuteQuery({
                query: updateTermMeta({term_id:termId,meta_key:'_menuitem_link',meta_value:body.link})
            })
            const updateTermMetaImageResult = await excuteQuery({
                query: updateTermMeta({term_id:termId,meta_key:'_menuitem_image',meta_value:body.term_image})
            })
