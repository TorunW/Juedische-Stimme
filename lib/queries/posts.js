...
const selectTranslations = ({ locale }) => {
  const query = `, (
                          ...
                          SELECT wp_postmeta.meta_value
                          FROM wp_postmeta
                          WHERE wp_postmeta.post_id=wp_posts.ID AND wp_postmeta.meta_key='_post_excerpt_2_translation_${locale}'
                          LIMIT 1
                        ) as post_excerpt_2_translation_${locale}, (
                            SELECT wp_postmeta.meta_value
                            FROM wp_postmeta
                            WHERE wp_postmeta.post_id=wp_posts.ID AND wp_postmeta.meta_key='_post_content_2_translation_${locale}'
                            LIMIT 1
                        ) as post_content_2_translation_${locale} `;
  return query;
};
...
export function selectPosts(body) {
  ...
  let postFields = ` wp_posts.*, wp_users.user_nicename as username , ${selectPostSubQueries(
    { locale }
  )}`;
  if (fieldsList && fieldsList.length > 0)
    postFields = generateFileds(fieldsList, locale);
  ...
  if (slug && slug !== null) {
    whereClause += `${isAnd ? "AND" : "WHERE"} wp_terms.slug='${slug}' ${
      isCategory === true ? " AND wp_term_taxonomy.taxonomy='category' " : ""
    }`;
    isAnd = true;
  }

  if (exclude && exclude.category) {
    whereClause += `${isAnd ? "AND" : "WHERE"} wp_terms.term_id!=${
      exclude.category
    }`;
    isAnd = true;
  }
...
export function countPostsByTag(body) {
  ...
  let whereClause = ``;
  if (slug && slug !== null) {
    whereClause = `WHERE wp_terms.slug='${slug}' ${
      isCategory === true ? " AND wp_term_taxonomy.taxonomy='category' " : ""
    }`;
...
export function selectPostsBySearchPhrase(body) {
  const { phrase, numberOfPosts, pageNum, locale, locales } = body;
  const query = `SELECT wp_posts.ID as postId, wp_posts.*, wp_users.user_nicename as username,
                        ${selectPostSubQueries({ locale, locales })}
                        FROM wp_posts 
                        LEFT JOIN wp_users ON wp_posts.post_author=wp_users.ID
...
export function selectNextPostInCategoryById({
  postId,
  categoryId,
  isPrevious,
}) {
