/* POSTS */
    ...
    const generateFileds = (fieldsList) => {
        let postFields = '';
    ...
    export function selectPosts(body){
        ...
        const { numberOfPosts,pageNum,showUnpublished,postType,fieldsList, exclude, locale } = body

        let postFields = ' wp_posts.* ,';
        if (fieldsList && fieldsList.length > 0) postFields = generateFileds(fieldsList)
    ...
    // select posts by args
    export function selectAdminPosts(body){
    ...
    export function selectPostByName(body){
        ...
        const query =   `SELECT wp_posts.ID as postId, wp_posts.*, wp_users.user_nicename as username,
                        ${selectPostSubQueries({locale,locales})}
                        , (
                            SELECT prev.post_name
                            from wp_posts prev
                            WHERE prev.ID < wp_posts.ID AND post_status='publish' AND post_name NOT LIKE '%newsletter%'
    ...
    export function selectPostsByTag(body){
        ...
        const { slug,numberOfPosts,pageNum,isCategory, fieldsList, locale } = body
