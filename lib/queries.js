...
/* /TAGS */
    ...
    const selectMenuItemsSnippet = `SELECT wp_posts.post_title, wp_posts.post_name, wp_posts.ID, wp_term_taxonomy.taxonomy, wp_term_taxonomy.description as title, wp_term_taxonomy.term_id, wp_termmeta.meta_value as link, wp_term_relationships.term_order
                                    FROM wp_terms
                                    INNER JOIN wp_term_taxonomy ON wp_term_taxonomy.term_id=wp_terms.term_id
    ...
    export function selectMenuItems(){
        const query =   `${selectMenuItemsSnippet}
                        WHERE wp_term_taxonomy.taxonomy IN ('footer_menu','main_menu','socials_menu','call_to_action_menu')
    ...
    export function selectNavItems(type){
        ...
        let inList = `('main_menu','socials_menu','call_to_action_menu')`
        if (type) inList = `('${type}')`

        const query =   `${selectMenuItemsSnippet}
                        WHERE wp_term_taxonomy.taxonomy IN ${inList}
                        ORDER BY wp_term_relationships.term_order`
        return query
    }
