...
const PostPageNavigation = ({ postId, categoryId }) => {
  ...
  async function getNextPrevPostNames() {
    const nextPostResponse = await axios.post(`/api/posts/${postId}/nextpost`, {
      categoryId: categoryId,
    });
    setNextPost(nextPostResponse.data);
    const prevPostResponse = await axios.post(`/api/posts/${postId}/prevpost`, {
      categoryId: categoryId,
    });
  ...
  return (
    <div className={styles.navigationContainer}>
      {nextPost && nextPost !== null ? (
        <Link href={"/" + nextPost.post_name.replaceAll("#", ":__--__:")}>
          <a title={nextPost.post_title}>
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
      ...
      {prevPost && prevPost !== null ? (
        <Link href={"/" + prevPost.post_name.replaceAll("#", ":__--__:")}>
          <a title={prevPost.post_title}>
            <div>Previous</div>
            <svg
              width="24"
              height="24"
