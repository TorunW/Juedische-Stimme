...
const FacebookEvents = ({ fbt }) => {
  ...
  async function fetchFacebookEvents() {
    ...
    if (fetchedEvents.data && fetchedEvents.data.length > 0) {
      const newEvents = [];
      fetchedEvents.data.map((fe, index) => {
        if (Date.parse(fe.start_time) > Date.now()) {
          newEvents.push(fe);
        }
      });
      // remove all the weird characters from the content to avoid mySql errors
      const renderedEvents = renderToString(newEvents);
  ...
  if (events?.content?.length > 0) {
    ...
    eventsDisplay = eventsArray.map((fbEvent, index) => {
      return (
        ...
        >
          <a
            href={`https://www.facebook.com/events/${fbEvent.id}`}
            target="_blank"
          ...
          >
            ...
            <p className={styles.title}>{fbEvent.name}</p>
